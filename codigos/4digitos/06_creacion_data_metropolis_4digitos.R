...
# Script: Data de Municipios-Metrópolis 4 digitos
...
#Cargar Datos del Censo Económico para todos los años, para todas las  Ramas, para todos los municipios, con totales

ce_mun<-read_csv("datos/4digitos/SAIC_Exporta_2025529_134917467.csv")
#############total_nacional_t<-ce_mun %>% filter(`Actividad económica` == "Total nacional")
...
#Crear la data con los datos a nivel nacional por año por cada Rama. Al nombrar la variable como Rama me estoy refiriendo a las Ramas a 4 dígitos "ce_nac" ----
ce_nac <- ce_mun %>%
  filter(Entidad == "00 Total Nacional")
#Renombrar las variables del ce_nac
ce_nac<-ce_nac%>%
  rename(
    ue_nac_Rama_t=`UE Unidades económicas`
  )
ce_nac<-ce_nac%>%
  rename(
    pot_nac_Rama_t= `H001A Personal ocupado total`
  )
ce_nac<-ce_nac%>%
  rename(
    vapercap_nac_Rama_t= `A204A Valor agregado en promedio por persona ocupada (Pesos)`
  )
ce_nac<- ce_nac%>%
  rename(
    ingresos_nac_Rama_t=`M000A Total de ingresos por suministro de bienes y servicios (millones de pesos)`
  )
ce_nac<- ce_nac%>%
  rename(
    ingresos_percap_nac_Rama_t=`A436A Ingresos por suministro de bienes y servicios por persona ocupada (Pesos)`
  )
ce_nac<- ce_nac%>%
  rename(
    vacb_nac_Rama_t=`A131A Valor agregado censal bruto (millones de pesos)`
  )
...
ce_nac<-ce_nac %>% select(`Año Censal`, `Actividad económica`, ue_nac_Rama_t, pot_nac_Rama_t, vacb_nac_Rama_t, vapercap_nac_Rama_t, ingresos_nac_Rama_t, ingresos_percap_nac_Rama_t)
#####
...
ce_mun_filtrado <- ce_mun_filtrado %>%
  filter(`Actividad económica` != "Total municipal")
glimpse(ce_mun_filtrado)

#Aquí tengo Tres Bases de datos----

#1.- Base de datos con las variables a nivel Rama a nivel nacional por año "ce_nac"
#2.- Base de datos con los totales municipales por año por cada variable "ce_tot_mun"
#3.- Base de datos con el el valor de la variable por Rama por municipio por año "ce_mun_filtrado"

#Quiero en una sola base de datos tener, por año, por metrópoli y por Rama las UE (y todas las variables)
...
#Ahora Sigue Sacar las variables por año, por ZM, por Rama----

#1.- Identificar Municipio Metropolitanos en la base ce_mun_filtrado
municipios_metro_con_Rama_anio <- merge(zm_2015, ce_mun_filtrado,
                                             by = "CVE_MUN",
                                             all = TRUE)
# Eliminar municipios que no son metropolitanos
municipios_metro_con_Rama_anio <- municipios_metro_con_Rama_anio %>%
  filter(!is.na(CVE_ZM))
#Ordenar las variables
municipios_metro_con_Rama_anio<-municipios_metro_con_Rama_anio %>%
  select(`Año Censal`, NOM_ZM, `Actividad económica`, everything())
View(municipios_metro_con_Rama_anio)

write_csv(municipios_metro_con_Rama_anio, "datos/4digitos/data_municipios_metro_con_Rama_anio.csv")
...
metro_sectores_año <- municipios_metro_con_Rama_anio %>%
...
metro_anio_sector_totales <- merge(metro_sectores_año, totales_metro_año,
                                   ...
                                   all = TRUE)
#Agregar tamaño nacional del sector en el año t de la base "ce_nac"----
...
metro_anio_sector_totales_secnac$cve_sector <- substr(metro_anio_sector_totales_secnac$`Actividad económica`, 6, 9)
...
#Eliminar de la base metropolitana a las Ramas que no forman parte del estudio, y que fueron descartados desde la elaboración del índice por no ser del tipo "establecimiento" o por carecer de datos en alguna de las bases utilizadas.
...
data_metropoli_sectores_intensidad<-data_anio_metro_sector_intensidad_k_filtrado %>% select(
  ...
  ue_nac_Rama_t, pot_nac_Rama_t, vacb_nac_Rama_t, ingresos_nac_Rama_t, vapercap_nac_Rama_t, ingresos_percap_nac_Rama_t,
  PC1, Indice_Normalizado,Media_Indice_Cluster, top_cluster)
#Terminada la Data Metropolitana por año, por metropoli, por Rama con intensidad del conocimiento----

#Guardar la base de datos----
write_csv(data_metropoli_sectores_intensidad, "datos/4digitos/data_metropoli_sectores_intensidad_4digitos_Ramas.csv")
